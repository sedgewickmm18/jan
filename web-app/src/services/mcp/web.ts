...
export class WebMCPService implements MCPService {
  ...
  async respondToElicitation(
    ...
    // For web platform, elicitation is handled by the remote API server
    console.log('respondToElicitation called:', { elicitationId, action, content })
    // No-op - web platform doesn't support direct elicitation
  }

  async checkJanBrowserExtensionConnected(): Promise<boolean> {
    // For web platform, browser extension is not applicable
    return false
  }
}
